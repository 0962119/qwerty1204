...
namespace BUS
{
    public class MonneyClass
    {
        ...
        public string FormatString(string gNum1)
        {
            string[] contan;
            contan = gNum1.Split('.');
            if (contan.Count() < 2)
            {
                return FormatString1(contan[0]);
            }
            else
            {
                string n = FormatString1(contan[0]);
                string tp = FormatString1(contan[1]);
                return n + "." + tp;
            }
        }
        public  string So_chu(string gNum1)
        {
            double gNum = double.Parse(gNum1);
            if (gNum == 0)
                return "Không đồng";
